...
database.ref().on("child_added", function (childSnapshot) {
    ...
    // with while loop we change this value every time
    var startTrainMoment = moment(trStart, "hh:mm A");
    // they are both moment objects
    console.log(now);
    console.log(startTrainMoment);

    // 06:00 am
    while (startTrainMoment.isBefore(now)) {

        startTrainMoment.add(trFrequency, 'm');
        

    }
    // format makes it as a string
    var nextTrain = startTrainMoment.format("hh:mm A");
    console.log(nextTrain);
    // we need to use startmoment which is actually already became a next train moment so we don't have a negative number

    var minAway = startTrainMoment.diff(now, "m");
